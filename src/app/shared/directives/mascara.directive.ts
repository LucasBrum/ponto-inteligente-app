...
export class MascaraDirective implements ControlValueAccessor {
  ...
  registerOnTouched(fn: any) {
   ...
   // Sempre que apertar uma tecla, ele vai disparar uma tecla
  @HostListener('keyup', ['$event'])
  onKeyup($event: any) {
    let valor: string = $event.target.value.replace(/\D/g, '');

    // retorna caso pressionado o backspace
    if ($event.keyCode === 8) {
      this.onChange(valor);
      return;
    }
  ...
  // Ao sair do campo, valida a mascara, se o valor for inválido, limpa o campo
